...
function activate(context) {
    ...
    let searchCommand = vscode.commands.registerCommand('extension.chickenNugget', function () {
        try {
          const inputPromise = vscode.window.showInputBox({ prompt: 'Enter keyword to search' });     
          inputPromise.then(keyword => {
            if (!keyword) return;
            ...
            try {
              ...
              vscode.window.showErrorMessage(`Invalid search pattern: ${regexError.message}`);
            }
          });
          
          // Add an error handler to the promise chain using void operator
          void inputPromise.then(null, error => {
